...
function updateTopbar(){
  ...
  if(xpCountEl) xpCountEl.textContent = xp.toLocaleString();
  
  const rank = getRank(xp);
  const rankBadgeEl = document.getElementById('rank-badge');
  if(rankBadgeEl) rankBadgeEl.textContent = `${rank.icon} ${rank.label}`;
...
function updateProgress(){
  ...
  const topicsDoneEl = document.getElementById('topics-done');
  if(topicsDoneEl) topicsDoneEl.textContent = done;
  
  const topicsTotalEl = document.getElementById('topics-total');
  if(topicsTotalEl) topicsTotalEl.textContent = total;
  
  const ringEl = document.getElementById('progress-ring');
  if(ringEl) {
    const circ = 2*Math.PI*20;
    ringEl.setAttribute('stroke-dashoffset', circ - (pct/100)*circ);
  }
...
function isTokenValid(token) {
  ...
  if (!decoded || !decoded.exp || !decoded.role || !decoded.id) return false;
...
async function checkAuth(){
  ...
  // No token → show landing page, do not show login overlay
  if(!token){
  ...
  // Pre-validate token structure and expiry before attempting API or Offline mode
  if (!isTokenValid(token)) {
    localStorage.removeItem('ceh_token');
    localStorage.removeItem('ceh_role');
    localStorage.removeItem('ceh_email');
    document.getElementById('landing-page').style.display = 'flex';
    document.getElementById('ceh-app').style.display = 'none';
    return;
  }
  ...
  try {
    ...
    const cachedEmail = localStorage.getItem('ceh_email');
    const cachedRole  = localStorage.getItem('ceh_role');
    if(cachedEmail && cachedRole){
      // We already validated token expiry above. Enter offline mode (read-only)
      bootApp(cachedEmail, cachedRole);
      // Notify user they are offline (read-only restricted operations check for connectivity implicitly)
      setTimeout(()=>{
        showToast('⚠️ Offline Mode (Read-Only) — Reconnect to sync progress');
      }, 1500);
    } else {
      // Missing cached data despite token
      document.getElementById('landing-page').style.display = 'flex';
      document.getElementById('ceh-app').style.display = 'none';
    }
  }
...
function showLogin(){
  const overlay = document.getElementById('ceh-login-overlay');
  ...
  MODULES.forEach((m, i) => {
    ...
    html += `
      <div class="lms-mod-card" onclick="toggleModule('${m.id}', document.getElementById('mod-hdr-${m.id}'))">
        ...
        <div style="display:flex;justify-content:space-between;font-size:0.75rem;color:var(--gfs-text-muted);margin-bottom:6px;font-family:var(--gfs-font-mono);">
...
function bootApp(email, role){
  ...
  document.getElementById('ceh-login-overlay').classList.remove('visible');
  ...
  // Show admin button for admin/superadmin roles
  if(role === 'admin' || role === 'superadmin'){
    const adminBtn = document.getElementById('btn-admin-nav');
    if(adminBtn) adminBtn.style.display = 'inline-flex';
  }

  // BUG-04 FIX: renderSidebar MUST run before updateTopbar so that
  // updateProgress() can find .topic-btn elements via querySelectorAll.
  renderSidebar();
  updateTopbar();
  initParticles();
  if(typeof renderLMSDashboard === 'function') renderLMSDashboard();

  // BUG-05: Sync progress from server on every session start
...
async function syncProgressFromServer() {
  ...
  try {
    ...
    if (r.ok) {
      ...
      // Merge local completed object with server state
      let serverCompleted = {};
      if (data.completed_topics && Array.isArray(data.completed_topics)) {
        data.completed_topics.forEach(t => {
          serverCompleted[t.topic_id] = true;
        });
      }
      
      // Schema validation implicitly handled by merging keys 
      let mergedCompleted = { ...completed, ...serverCompleted };
      
      // Conflict Detection & Push Missing Topics
      let missingOnServer = [];
      for (const tid of Object.keys(completed)) {
        if (!serverCompleted[tid]) {
          missingOnServer.push(tid);
        }
      }
      
      let finalXP = data.total_xp || 0;
      
      // If there are offline completed topics, push them to server now
      if (missingOnServer.length > 0) {
        for (const tid of missingOnServer) {
          await fetch('/api/progress/topic', {
            method: 'POST',
            headers: {'Content-Type': 'application/json', 'Authorization': 'Bearer ' + token},
            body: JSON.stringify({topic_id: tid, xp_earned: 200, flag_captured: false})
          });
        }
        
        // Server Recalculate: Fetch progress again to get the final authoritative server state
        const r2 = await fetch('/api/progress', {headers: {'Authorization': 'Bearer ' + token}});
        if (r2.ok) {
           const data2 = await r2.json();
           finalXP = data2.total_xp || 0;
        }
      }
      
      // Update Local and Verify Match (trusting server's recalculation)
      ...
      // Re-render UI with synced state
      updateTopbar();
      if(typeof renderLMSDashboard === 'function') renderLMSDashboard();
    }
  } catch(e) {
    console.warn("Offline: Could not sync progress from server.");
  }
}
...
function buildStandardizedCommands(cmds) {
  if (!cmds) return '';
...
function buildDiagramHTML(d){
  ...
  return `
  <div style="background:var(--gfs-bg-panel);border:1px solid var(--gfs-border);border-radius:var(--gfs-radius);padding:28px;">
    <div style="font-size:1rem;font-weight:700;color:var(--gfs-info);margin-bottom:24px;font-family:var(--gfs-font-mono);">${d.diagram.title||'Process Flow'}</div>
...
function buildEnterpriseHTML(d){
  ...
  <div style="background:var(--gfs-bg-panel);border:1px solid var(--gfs-border);border-radius:var(--gfs-radius);padding:28px;">
    <div style="display:flex;align-items:center;gap:14px;margin-bottom:24px;">
      ...
      <div>
        <div style="font-size:0.7rem;font-family:var(--gfs-font-mono);color:var(--gfs-success);letter-spacing:0.12em;">ENTERPRISE SCENARIO — GLOBALFINSEC CORP</div>
        <div style="font-size:1rem;font-weight:700;color:var(--gfs-text-primary);">${e.role||'Lead Penetration Tester'}</div>
      </div>
    </div>
    ...
    ${e.steps&&e.steps.length?`<div style="margin-top:20px;">
      <div style="font-family:var(--gfs-font-mono);font-size:0.72rem;color:var(--gfs-text-muted);letter-spacing:0.1em;margin-bottom:12px;">// YOUR INVESTIGATION STEPS</div>
      ${e.steps.map((s,i)=>`<div style="display:flex;gap:12px;align-items:flex-start;margin-bottom:10px;padding:12px;background:rgba(0,0,0,0.3);border-radius:8px;border-left:3px solid var(--gfs-success);"><span style="font-family:var(--gfs-font-mono);color:var(--gfs-success);font-size:0.75rem;min-width:20px;">${i+1}.</span><span style="font-size:0.88rem;color:var(--gfs-text-primary);">${s}</span></div>`).join('')}
    </div>`:''}
...
function buildCommandsHTML(d){
      ...
      tools.map(t => {
        ...
        <div class="tool-card">
          ...
          <div class="tool-body">
            ...
            <div class="tool-col">
              <div class="tool-lbl" style="color:var(--gfs-primary)">✨ AI-POWERED EVOLUTION</div>
  ...
  <div class="cmd-grid">
    <div class="cmd-block">
      ...
      ${cmds.win&&cmds.win.length?cmds.win.map(c=>`<div class="cmd-line clickable-cmd" onclick="runCommand('win', '${c.replace(/'/g,"\'").replace(/"/g,'&quot;')}')">${c}</div>`).join(''):'<div style="color:var(--gfs-text-secondary);font-size:0.8rem;padding:8px;">No Windows commands.</div>'}
    </div>
    <div class="cmd-block">
      <div class="cmd-os">▶ LINUX / KALI COMMANDS (Click to execute)</div>
      ${cmds.lin&&cmds.lin.length?cmds.lin.map(c=>`<div class="cmd-line clickable-cmd" onclick="runCommand('lin', '${c.replace(/'/g,"\'").replace(/"/g,'&quot;')}')">${c}</div>`).join(''):'<div style="color:var(--gfs-text-secondary);font-size:0.8rem;padding:8px;">No Linux commands.</div>'}
    </div>
  </div>`;
}
// -- TAB 5: PITFALLS --
function buildPitfallsHTML(d){
  ...
  return `<div style="display:flex;flex-direction:column;gap:16px;">${d.pitfalls.map((p,i)=>`
    <div style="background:rgba(255,59,92,0.07);border:1px solid rgba(255,59,92,0.25);border-radius:var(--gfs-radius);padding:20px;">
      <div style="display:flex;align-items:center;gap:10px;margin-bottom:10px;">
        <span style="font-size:1.3rem;">${p.icon||'\u26A0'}</span>
        <span style="font-weight:700;color:var(--gfs-danger);font-size:0.95rem;">${p.title}</span>
        <span style="margin-left:auto;font-family:var(--gfs-font-mono);font-size:0.65rem;background:rgba(255,59,92,0.15);color:var(--gfs-danger);padding:3px 8px;border-radius:20px;">PITFALL #${i+1}</span>
      </div>
      <div style="font-size:0.87rem;color:var(--gfs-text-primary);margin-bottom:10px;">${p.desc}</div>
      <div style="background:rgba(0,200,83,0.08);border:1px solid rgba(0,200,83,0.2);border-radius:8px;padding:10px;">
        <span style="font-family:var(--gfs-font-mono);font-size:0.7rem;color:var(--gfs-success);">\u2713 FIX: </span>
        <span style="font-size:0.84rem;color:var(--gfs-text-muted);">${p.fix}</span>
      </div>
    </div>
  `).join('')}</div>`;
}
...
function buildSummaryHTML(id,d){
  ...
  return `
  <div style="background:var(--gfs-bg-panel);border:1px solid var(--gfs-border);border-radius:var(--gfs-radius);padding:28px;">
    <div style="font-size:1rem;font-weight:700;color:var(--gfs-success);margin-bottom:20px;font-family:var(--gfs-font-mono);">// KEY TAKEAWAYS — EXAM READY</div>
    ${d.summary&&d.summary.length?`<ul class="summary-list">${d.summary.map(s=>`<li>${s}</li>`).join('')}</ul>`:'<div style="color:var(--gfs-text-secondary);">Summary content coming soon.</div>'}
    <div style="margin-top:32px;padding-top:24px;border-top:1px solid var(--gfs-border);display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:12px;">
      <div id="complete-status-${id}" style="font-family:var(--gfs-font-mono);font-size:0.8rem;color:var(--gfs-text-secondary);">
        ${isDone?'<span style="color:var(--gfs-success);">\u2713 TOPIC COMPLETED — XP AWARDED</span>':'Complete all sections, then mark done to earn XP.'}
      </div>
...
function buildFlashcardsHTML(d){
  ...
  return `
  <div style="margin-bottom:10px;font-family:var(--gfs-font-mono);font-size:0.75rem;color:var(--gfs-text-muted);">// Hover a card to flip and reveal the definition</div>
...
async function showAdminPanel(){
  ...
  try {
    ...
    users.forEach(u => {
      ...
      const roleHtml = u.role === 'admin' ? '<span style="color:var(--gfs-danger);border:1px solid var(--gfs-danger);padding:2px 6px;border-radius:4px;font-family:var(--gfs-font-mono);font-size:0.6rem;">ADMIN</span>' : '<span style="color:var(--cyan);border:1px solid var(--cyan);padding:2px 6px;border-radius:4px;font-family:var(--gfs-font-mono);font-size:0.6rem;">OPERATOR</span>';
      trs += `<tr style="border-bottom:1px solid rgba(255,255,255,0.05);"><td style="padding:16px;">${u.email}</td><td style="padding:16px;">${roleHtml}</td><td style="padding:16px;color:var(--gfs-success);font-family:var(--gfs-font-mono);">${u.total_xp || 0}</td><td style="padding:16px;">${u.tasks_completed || 0}</td><td style="padding:16px;color:var(--gfs-text-muted);font-size:0.7rem;">${new Date(u.created_at).toLocaleDateString()}</td></tr>`;
    });
...
const SCRIPT = [
  ...
  { c: "var(--gfs-text-muted)", t: "Starting Nmap 7.94 ( https://nmap.org )" },
...
setInterval(() => {
  ...
  el.innerHTML = `<div style="display:flex;justify-content:space-between;background:rgba(0,0,0,0.4);padding:4px 8px;border-radius:4px;font-family:var(--gfs-font-mono);font-size:0.6rem;animation:pulse-glow 1s;"><span style="color:${a.c}">${a.sev}</span><span style="color:#fff">${a.t}</span></div>` + el.innerHTML;
...
async function markComplete(topicId){
  ...
  try{
    ...
    if(r.ok){
      ...
      if(st) st.innerHTML='<span style="color:var(--gfs-success);">✓ TOPIC COMPLETED — XP AWARDED</span>';
