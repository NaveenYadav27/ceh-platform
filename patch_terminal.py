...
def patch_terminal(file_path):
    with open(file_path, 'r', encoding='utf8') as f:
        html = f.read()

    # Find handleTerminalInput
    pattern = r'function handleTerminalInput\(e, os, inputEl\).*?\n\}'
    
    replacement = """window.escapeHtml = function(unsafe) {
  ...
  if(e.key === 'Enter'){
    ...
    setTimeout(() => {
      ...
      if(lowerCmd === 'clear' || lowerCmd === 'cls'){
        output = ""; 
      } else if(TERMINAL_CMDS && TERMINAL_CMDS[lowerCmd]){
    ...
    if not re.search(pattern, html, re.DOTALL):
        print(f"Failed to find handleTerminalInput in {file_path}")
        return
        
    new_html = re.sub(pattern, replacement, html, flags=re.DOTALL)
    
    # Just in case executeCommand is there
    new_html = new_html.replace('executeCommand(', "runCommand('lin', ")
    
    with open(file_path, 'w', encoding='utf8') as f:
        f.write(new_html)
    print(f"Patched {file_path}")

patch_terminal('frontend/index.html')
patch_terminal('frontend/index.min.html')
