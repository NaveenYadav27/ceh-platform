...
def fix_all_orphans(file_path):
    with open(file_path, 'r', encoding='utf8') as f:
        html = f.read()

    # The corrupted block starts with window.escapeHtml and ends right before function wireCTF(id, d){}
    # In index.min.html and index.html, we can just replace everything from window.escapeHtml to // ── CTF ──
    pattern = r'window\.escapeHtml = function.*?// ── CTF ──'
...
// ── CTF ──"""
    ...
    with open(file_path, 'w', encoding='utf8') as f:
        f.write(new_html)
    print(f"Cleanly patched {file_path}")

fix_all_orphans('frontend/index.html')
fix_all_orphans('frontend/index.min.html')
